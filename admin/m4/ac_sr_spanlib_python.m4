...
AC_DEFUN([AC_SR_SPANLIB_PYTHON],[
	...
	# So..
	AS_VAR_SET(WITH_PYTHON,
		[`AS_VAR_SET(HAS_PYT_MOD) && test "AS_VAR_GET(PYFORT)" != "no" -a \
			"AS_VAR_GET(PERL)" != "no" -a "AS_VAR_GET(HAS_BLASLAPACK)" != "no"`])

	AM_CONDITIONAL([WITH_PYTHON],AS_VAR_GET(WITH_PYTHON))
	AS_IF(AS_VAR_GET(WITH_PYTHON),,
...
You need perl, BLAS/LAPACK, pyfort and Numeric, MV python modules from CDAT.])]
	...
	AS_VAR_SET(WITH_PYTHON_EXAMPLE,[false])
	AS_IF(AS_VAR_GET(WITH_PYTHON),[
		AS_VAR_GET(PYTHON) -c ["import cdms;import vcs"] 2> /dev/null
		AS_IF([test "$?" = "0"],
			[AC_MSG_RESULT([yes])
			AS_VAR_SET(WITH_PYTHON_EXAMPLE,[`true`])]
		)
		AS_IF(AS_VAR_GET(WITH_PYTHON),,
			[AC_SR_WARNING([You wont be able to run the python example.
You need cnds and vcs python modules from CDAT.])]
		)
	])
	AM_CONDITIONAL([WITH_PYTHON_EXAMPLE],AS_VAR_SET(WITH_PYTHON_EXAMPLE))
